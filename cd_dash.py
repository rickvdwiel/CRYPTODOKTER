...
def api_chart() -> dict:
    ...
    if not points:
        ...
        banked = round(float(getattr(pf, "banked_eur", 0.0) or 0.0), 4)
        points = [{
            "t": "start",
            "equity": eq,
            "cash": pf.cash_eur,
...
def serve(host: str = "127.0.0.1", port: int = 8000) -> int:
    ...
    guard = HyperInstanceGuard(owner=f"cd_dash:{host}:{port}")
    ...
    print("   PAPER ONLY · ONE hyper instance (cd_dash / web.server / scheduler)")
